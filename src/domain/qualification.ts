...
export function qualifyCandidateDetailed(candidate: CampaignCandidate, searchPackage: SearchPackage): QualificationResult {
  ...
  const quoteReview = candidate.quote.total > searchPackage.budgetCeilingPerCreator;
  const gates: GateResult[] = [
    ...
    gate("commercial", "版权、档期与预算", quoteReview ? "Review" : "Pass", "matrix.commercial", [], quoteReview ? "报价超出单元格上限" : "报价和档期可执行"),
  ];
  ...
  const risks = [...gates.filter((item) => item.status === "Review").map((item) => item.summary), ...(candidate.quote.status === "Estimated" ? ["报价仍为估价，发布前需确认"] : [])];
