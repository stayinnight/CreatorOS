...
export function qualifyCandidate(candidate: CampaignCandidate, searchPackage: SearchPackage): Qualification {
  ...
  return { status: commercialOnlyReview ? "Qualified" : result.status, reasons: commercialOnlyReview ? [] : result.gates.filter((item) => item.status !== "Pass").map((item) => item.summary), risks: result.risks };
}
