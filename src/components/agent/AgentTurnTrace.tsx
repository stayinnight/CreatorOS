...
import { localizeToolStep, localizeUnderstanding } from "../../i18n/agentCopy";
...
export function AgentTurnTrace({ phase, understanding, steps, completedStepCount }: { phase: AgentTurnPhase; understanding: string; steps: AgentToolStepRecord[]; completedStepCount: number }) {
  ...
  return <section className="agent-turn-trace" role="status" aria-label={locale === "zh-CN" ? "Agent 执行状态" : "Agent execution status"} aria-live="polite"><header><span className="working-mark" /><div><strong>{t(phaseKey)}</strong><small>{localizeUnderstanding(locale, understanding)}</small></div></header>{phase !== "Understanding" && <div className="agent-tool-list">{localizedSteps.map((step, index) => { const status = index < completedStepCount ? "Done" : index === completedStepCount && phase === "RunningTools" ? "Running" : "Pending"; return <div className={`agent-tool-step is-${status.toLowerCase()}`} key={step.id}><i>{status === "Done" ? "✓" : status === "Running" ? <span className="working-mark" /> : "·"}</i><span>{step.label}</span><em>{status}</em></div>; })}</div>}</section>;
}
