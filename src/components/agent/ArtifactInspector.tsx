...
export function ArtifactInspector({ artifact, phase, revealKey, onClose, onOpenArtifact, onAskAgent, onPerformAction }: { artifact: CampaignArtifact; phase: ArtifactTransitionPhase; revealKey: number; onClose: () => void; onOpenArtifact: (artifactId: string) => void; onAskAgent?: (prompt: string, context?: { candidateId?: string }) => void; onPerformAction?: (action: CampaignAction) => boolean }) {
  ...
  return <aside ref={inspectorRef} className={`artifact-inspector is-${phase}`} aria-label={`${formatDomainValue(locale, artifact.kind)} ${locale === "zh-CN" ? "检查器" : "inspector"}`}><header className="inspector-head"><div><span>{formatDomainValue(locale, artifact.kind).toUpperCase()} · {locale === "zh-CN" ? "版本" : "VERSION"} {artifact.version}</span><h2 ref={headingRef} tabIndex={-1}>{localizeSystemText(locale, artifact.summary)}</h2></div><button type="button" aria-label={locale === "zh-CN" ? "关闭产物" : "Close artifact"} onClick={onClose}>×</button></header>{next && <RecommendedNextStep action={next} onExecute={execute} />}<div className="inspector-content" key={artifact.id}>{artifact.kind === "Brief" ? <BriefArtifactDetail /> : artifact.kind === "Mix" ? <MixArtifactDetail scenarioId={artifact.domainRef} /> : artifact.kind === "SearchPackageSet" ? <SearchPackageSetDetail /> : artifact.kind === "CandidateBatch" ? <CandidateBatchDetail artifact={artifact} onAskAgent={onAskAgent} onPerformAction={onPerformAction} /> : artifact.kind === "ReviewRound" ? <ReviewRoundDetail /> : artifact.kind === "GapAssessment" ? <GapArtifactDetail /> : <div className="inspector-placeholder"><strong>{formatDomainValue(locale, artifact.kind)} {locale === "zh-CN" ? "详情" : "detail"}</strong></div>}</div></aside>;
}
...
function SearchPackageSetDetail() {
  const { state } = useCampaign();
  const { locale } = useLanguage();
  return <div className="inspector-body"><section className="inspector-section"><span className="micro-label">{locale === "zh-CN" ? "有边界的搜索" : "BOUNDED SEARCH"}</span><h3>{locale === "zh-CN" ? `${state.searchPackages.length} 个任务包继承 Matrix 约束` : `${state.searchPackages.length} packages inherit Matrix constraints`}</h3><div className="compact-package-grid">{state.searchPackages.map((item) => <article key={item.id}><span>{item.market} · {item.platform}</span><strong>{formatDomainValue(locale, item.ridingScenario)} / {formatDomainValue(locale, item.contentFormat)}</strong><small>{item.candidateTargetCount} {locale === "zh-CN" ? "位候选人 · 上限" : "candidates · ceiling"} ${item.budgetCeilingPerCreator.toLocaleString(locale)}</small></article>)}</div></section><div className="inspector-approval"><div><strong>{locale === "zh-CN" ? "扩量前先校准" : "Calibrate before scaling"}</strong><p>{locale === "zh-CN" ? "运行 10 个样例：8 个合格方向和 2 个可见失败案例。使用上方建议动作继续。" : "Run a 10-case sample: 8 qualified directions and 2 visible failure cases. Use the recommended action above to continue."}</p></div></div></div>;
}
