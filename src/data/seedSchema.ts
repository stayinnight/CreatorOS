...
const agentSchema = z.object({
  ...
  messages: z.array(z.object({ id: z.string(), runId: z.string().nullable(), role: enumValues(["User", "Agent", "System"]), type: enumValues(["Text", "Plan", "RunGroup", "Decision", "Artifact", "Exception", "NextAction", "Progress"]), text: z.string(), payloadRef: z.string().nullable(), createdAt: z.string(), messageKey: z.string().optional(), messageParams: z.record(z.string(), z.union([z.string(), z.number()])).optional() })),
  ...
  calibrationReviews: z.array(z.object({ candidateId: z.string(), decision: enumValues(["Accepted", "Rejected", "NeedsFollowUp"]), reason: enumValues(["No real cycling", "Too commercial", "Not enough POV evidence", "Audience mismatch", "Quote too high"]).nullable(), reviewedAt: z.string() })).optional().default([]),
  calibrationPreferences: z.array(z.object({ id: z.string(), label: z.string(), strength: z.number(), affectedCandidateIds: z.array(z.string()) })).optional().default([]),
  calibrationPreferenceConfirmed: z.boolean().optional().default(false), calibrationSelectedCandidateId: z.string().nullable().optional().default(null),
  approvedPrimaryIds: z.array(z.string()).optional().default([]), approvedBackupIds: z.array(z.string()).optional().default([]),
  turns: z.array(z.object({ id: z.string(), query: z.string(), status: enumValues(["Completed", "Failed"]), understanding: z.string(), steps: z.array(z.object({ id: z.string(), kind: enumValues(["ReadSources", "ExtractBrief", "DetectConflicts", "ApplyDecision", "BuildMix", "LockMatrix", "GenerateSearchPackages", "EvaluateQualification", "InspectEvidence", "BuildCalibration", "ValidateSlate", "PublishReview", "AssessGap", "RecoverGap", "ReadMatrixCell", "SummarizeFit", "ReadQuote", "CompareBudgetCeiling", "ApplyCampaignPreferences", "ReadWorkflowState"]), label: z.string(), inputRefs: z.array(z.string()), summary: z.string() })), ruleIds: z.array(z.string()), evidenceIds: z.array(z.string()), answerMessageId: z.string().nullable(), error: z.string().nullable() })).optional().default([]),
});
