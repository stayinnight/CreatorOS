...
const agentSchema = z.object({
  ...
  messages: z.array(z.object({ id: z.string(), runId: z.string().nullable(), role: enumValues(["User", "Agent", "System"]), type: enumValues(["Text", "Plan", "RunGroup", "Decision", "Artifact", "Exception", "NextAction", "Progress"]), text: z.string(), payloadRef: z.string().nullable(), createdAt: z.string() })),
  ...
  turns: z.array(z.object({ id: z.string(), query: z.string(), status: enumValues(["Completed", "Failed"]), understanding: z.string(), steps: z.array(z.object({ id: z.string(), kind: enumValues(["ReadMatrixCell", "EvaluateQualification", "InspectEvidence", "SummarizeFit", "ReadQuote", "CompareBudgetCeiling", "ApplyCampaignPreferences", "ReadWorkflowState"]), label: z.string(), inputRefs: z.array(z.string()), summary: z.string() })), ruleIds: z.array(z.string()), evidenceIds: z.array(z.string()), answerMessageId: z.string().nullable(), error: z.string().nullable() })).optional().default([]),
});
